...
Add `-IncludeHidden` only for the separate hidden-backup render. The output uses
physical filenames such as `slide-006.png`, so visible-page gaps stay explicit.
...
- Requested hidden backups were rendered separately from scratch and remain
  hidden in the deliverable.
